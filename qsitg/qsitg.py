...
class Qsitg:
    ...
    def initGui(self):
        ...
        web_icon = QIcon(os.path.join(os.path.dirname(__file__), "resources", "catalog.png"))
        icon_enum = QStyle.StandardPixmap if hasattr(QStyle, "StandardPixmap") else QStyle
        help_icon = QApplication.style().standardIcon(icon_enum.SP_TitleBarContextHelpButton)
        ...
        self.action_catalog = QAction(web_icon, "Ouvrir le portail du SITG dans le navigateur")
        self.action_catalog.triggered.connect(self.run_open_catalog)
        self.menu.addAction(self.action_catalog)

        self.action_guide = QAction(help_icon, "Ouvrir le guide d'utilisation dans le navigateur")
    ...
    def run_about(self):
        """Open and display the plugin about dialog."""
        ...
        # Run the dialog
        self.dialog.dont_show_again.setChecked(self.settings.contains(KEY_DONT_SHOW_AGAIN))
        self.dialog.show()
        self.dialog.exec()
    ...
    def run_open_catalog(self):
        """Open the SITG catalog website in the default web browser."""
    ...
    def do_reset_geoservices(self, with_auth: bool):
        """Configure and reload SITG geoservices in QGIS."""
        browser_items_names = []
        ...
        for browser_item_name in browser_items_names:
            tree_items = model.match(
                ...
                browser_item_name,
                flags=Qt.MatchFlag.MatchRecursive | Qt.MatchFlag.MatchExactly | Qt.MatchFlag.MatchCaseSensitive,
            )
            for tree_item in tree_items:
                treeview.scrollTo(tree_item)
                treeview.selectionModel().select(tree_item, QItemSelectionModel.SelectionFlag.Select)
